...
BEGIN
	...
	IF NOT EXISTS(SELECT UserID FROM dbo.Users WHERE [Email] = @Email AND [Password] = @Password)
	BEGIN
	...
	BEGIN
		...
		DECLARE @UserID uniqueidentifier = (SELECT UserID FROM dbo.Users WHERE [Email] = @Email AND [Password] = @Password)
